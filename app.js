...
function ledger(){let l=$('#ledger');l.innerHTML=data.ledger.length?'':'<p class="hint">本月還沒有記帳。</p>';[...data.ledger].reverse().forEach((x,ri)=>{let i=data.ledger.length-1-ri,d=document.createElement('div');d.className='ledger';d.innerHTML=`<div><b>${escapeHtml(x.note||x.category)}</b><small>${escapeHtml(x.category)}・${escapeHtml(x.method||'現金')}・${escapeHtml(x.date)}</small></div><div><b>${fmt(x.amount)}</b> <button>刪除</button></div>`;d.querySelector('button').onclick=()=>{if(x.sourceId){cardTxs=cardTxs.filter(t=>t.id!==x.sourceId);localStorage.setItem(CARD_KEY,JSON.stringify(cardTxs))}data.ledger.splice(i,1);save()};l.appendChild(d)})}

// ----- 信用卡 -----
const CARD_KEY='liyunjia-creditcards-v1';
const CARD_SETTINGS_KEY='liyunjia-creditcard-settings-v3';
const cardDefaults={ctbcCarry:173114,fubonInst1Amount:4290,fubonInst1Count:2,fubonInst2Amount:1260,fubonInst2Count:1,yuniFubonCloseDay:8,cathayCloseDay:17};
function loadCardSettings(){let old={};try{old=JSON.parse(localStorage.getItem('liyunjia-creditcard-settings-v2')||'null')||{}}catch(e){}try{let x=JSON.parse(localStorage.getItem(CARD_SETTINGS_KEY)||'null');if(x)return {...cardDefaults,...old,...x}}catch(e){}return {...cardDefaults,...old}}
...
const CARDS={ctbc:{name:'中國信託',closeDay:()=>25},fubon:{name:'台北富邦',closeDay:()=>24},yuni_fubon:{name:'芋泥台北富邦',closeDay:()=>+cardSettings.yuniFubonCloseDay||0},cathay:{name:'國泰銀行',closeDay:()=>+cardSettings.cathayCloseDay||0}};
...
function renderLoans(){
  $('#loanTotal').textContent=fmt(totalOwed());$('#cashOnHandView').textContent=fmt(loans.cash);$('#cashOnHand').value=loans.cash;
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const regs=await navigator.serviceWorker.getRegistrations();for(const r of regs){if(!String(r.active?.scriptURL||'').includes('service-worker.js?v=12.0.0'))await r.unregister()}}catch(e){}try{await navigator.serviceWorker.register('./service-worker.js?v=12.0.0',{updateViaCache:'none'})}catch(e){}})}
render();
