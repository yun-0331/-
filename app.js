...
let cur={y:2026,m:9};
const defaults=[['先生生活費',12000],['孝親費',12000],['保險',16500],['信貸(1)',7000],['信貸(2)',6000],['信貸(3)',7500],['分期／卡費',9390],['補習與英文',17100],['ETC 與加油',9000],['電話',4000],['長照',1500],['捐款與 ETF',1600]];
const key=()=>`liyunjia-${cur.y}-${String(cur.m).padStart(2,'0')}`;
const fresh=()=>({income:{husband:67000,wife:35000,other:8000},expenses:defaults.map(([name,amount])=>({name,amount})),ledger:[],finished:false,bufferTarget:null,step:3});
function load(){let x;try{x=JSON.parse(localStorage.getItem(key())||'null')}catch(e){};x=x||fresh();if(x.bufferTarget===undefined)x.bufferTarget=null;if(!x.step)x.step=3;return x}
let data=load();
...
function expenses(){let g=$('#expenses');g.innerHTML='';data.expenses.forEach((x,i)=>{let d=document.createElement('div');d.className='expense';d.innerHTML=`<span>${x.name}</span><input type="number" inputmode="numeric" value="${x.amount}"><button aria-label="刪除">×</button>`;d.querySelector('input').onchange=e=>{data.expenses[i].amount=+e.target.value||0;save()};d.querySelector('button').onclick=()=>{if(confirm(`刪除「${x.name}」？`)){data.expenses.splice(i,1);save()}};g.appendChild(d)})}
...
$('#eSave').onclick=e=>{let name=$('#eName').value.trim(),amount=+$('#eAmount').value||0;if(!name||amount<=0){e.preventDefault();return alert('請輸入名稱與金額')}data.expenses.push({name,amount});data.finished=false;$('#eName').value='';$('#eAmount').value='';save()};
